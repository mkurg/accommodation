...
for root, dirs, files in os.walk('./exthdd/kribrum/twitter_1/2015/11/'):
    for name in files:
        if name.endswith('.gz'):
...
with gzip.open('../not_single_tweets/first1.gz', 'wt') as f:
    f.write(json.dumps(not_single_tweets))
with gzip.open('../tweets_by_repliers/first1.gz', 'wt') as f:
    f.write(json.dumps(tweets_by_repliers))